...
class Mandalaexchange extends Driver {
    ...
    async fetchTickers(isMocked) {
        ...
        const { data: { list: markets } } = await request('https://trade.mandala.exchange/open/v1/common/symbols');

        const tickers = throttleMap(markets, async(market) => {
            try {
                const base = market.baseAsset;
                const quote = market.quoteAsset;
                const details = await request(`https://trade.mandala.exchange/v1/market/trading-pairs?baseAsset=${base}&quoteAsset=${quote}`)
                const [, , , , , , , , , , , baseVolume, , quoteVolume, , low, high, open, close, ] = details.data.list[0];
