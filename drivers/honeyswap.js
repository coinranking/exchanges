 ...
 * @augments Driver
 */
class Honeyswap extends Driver {
  /**
  ...
  async blockNumber24hAgo(isMocked) {
    ...
    if (isMocked) {
      ...
      timestampYesterdayInSeconds = 1606043764;
    }
  ...
  async fetchTickers(isMocked) {
    ...
    const { data: { pairs } } = await request({
      method: 'POST',
      url: 'https://api.thegraph.com/subgraphs/name/kirkins/honeyswap',
      json: {
        query: `
        {
          pairs(
